(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.wsubstr = {}));
  ...
  function computed(span, text, childStr, maxWidth, average) {
    ...
    if (w < maxWidth || equal(w, maxWidth)) {
      // 字符穿宽度 <= maxWidth
      let d = maxWidth - w;

      if (d < average) {
        // 当前字符串宽度与最大宽度误差在一个平均字符宽度以内
        return {
          done: true,
          diff: 0
        };
      } else {
        // 当前字符串宽度与最大宽度误差超过一个字符
        return {
          done: false,
  ...
  function split(span, text, str, start, end, maxWidth, average) {
    ...
    let {
      ...
      diff
    } = computed(span, text, childStr, maxWidth, average);
  ...
  function wsubstr(str, maxWidth) {
    let css = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
    let temp = []; // 缓存切割后的字符串

    let {
      span,
      text
    } = createNode(str, css); // 生成计算字符宽高的容器

    let {
      average,
      size
    } = basicData(span, str, maxWidth); // 获取基础数据

    let start = 0,
        end = size; // 切割字符时的 开始/结束 索引
  ...
  exports.default = wsubstr;

  Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=wsubstr.js.map
